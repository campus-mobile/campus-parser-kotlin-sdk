...
abstract class BaseParser(
    ...
    protected suspend fun <T, R> parallelProcessing(
        ...
        // замена атомика, потому что atomicfu тут сломался
        val processedCounter: MutableStateFlow<Int> = MutableStateFlow(0)
                ...
                .map { item ->
                    async {
                        try {
                            ...
                            val processedCount: Int = processedCounter.updateAndGet { it + 1 }
    ...
    protected fun generateSchedules(
        ...
        val startOfWeek: LocalDate = currentDate.minus(DatePeriod(days = currentDate.dayOfWeek.ordinal))
